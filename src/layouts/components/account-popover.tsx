  ...
  return (
    <>
      <IconButton
        ...
        {...other}
      >
        <Avatar src={user?.nombre ? undefined : _myAccount.photoURL} alt={user?.nombre || _myAccount.displayName} sx={{ width: 1, height: 1 }}>
          {(user?.nombre || _myAccount.displayName).charAt(0).toUpperCase()}
        </Avatar>
      </IconButton>
      ...
      >
        <Box sx={{ p: 2, pb: 1.5 }}>
          <Typography variant="subtitle2" noWrap>
            {user?.nombre || _myAccount?.displayName}
          </Typography>

          <Typography variant="body2" sx={{ color: 'text.secondary' }} noWrap>
            {user?.correo || _myAccount?.email}
          </Typography>
        </Box>
